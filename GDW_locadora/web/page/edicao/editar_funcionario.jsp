    ...
    <body>        
        ...
        <div class="container">
            ...
            <div class="row" id="form-editar">
                <div class="col-md-12">
                    <h2 class="col-sm-offset-2">Editar Funcionário</h2>
                    <form class="form-horizontal" action="Controller" method="POST">
                        ...
                        <div class="form-group">
                            <label for="inputDataAdmissao" class="col-sm-2 control-label">Data de Admissão: </label>
                            <div class="col-sm-6">
                                <input type="text" id="inputDataAdmissao" class="form-control" name="data_admissao" placeholder="Data de Admissão"/>
                            </div> 
                        </div>
                        <div class="form-group">
                            <label for="inputSalario" class="col-sm-2 control-label">Salário: </label>
                            <div class="col-sm-6">
                                <input type="text" id="inputSalario" class="form-control" name="salario" placeholder="Salário"/>
                            </div> 
                        </div>
                        <div class="form-group">                            
            ...
            $(document).ready(function () {
                ...
                $("#pesquisarClientePorCPF").click(function () {
                    if ($("#inputPesquisaCPF").val() === "") {
                        ...
                        $.ajax({
                            url: "Controller?acao=AjaxBuscarFuncionarioPorCPF",
                            type: 'POST',
                            data: {cpf: $("#inputPesquisaCPF").val()},
                            datatype: 'json',
                            beforeSend: function () {
                                $("#aviso").remove();
                                $("#form-editar").prepend('<div id="aviso"><img src="library/img/ajax-loader.gif" title="carregado..." alt="Carregando..." /></div>');
                            },
                            success: function (funcionario) {
                                if (Object.keys(funcionario).length > 1) {
                                    $("#aviso").remove();
                                    $("#inputID").val(funcionario.id);
                                    $("#inputNome").val(funcionario.nome);
                                    $("#inputDataNascimento").val(funcionario.data_nascimento);
                                    $("#inputCPF").val(funcionario.cpf);
                                    $("#inputRG").val(funcionario.rg);
                                    $("#inputEndereco").val(funcionario.endereco);
                                    $("#inputTelefone").val(funcionario.telefone);
                                    $("#inputDataAdmissao").val(funcionario.data_admissao);
                                    $("#inputSalario").val(funcionario.salario);
                                } else {
                                    $("#aviso").remove();
                                    $("#form-editar").prepend('<div id="aviso" class="alert alert-danger" role="alert">' + funcionario.erro + '</div>');
                                }
